...
if not defined MAUL_REGIONINFO_URL set "MAUL_REGIONINFO_URL=https://moddedamong.us/data/regioninfo.json"
if not defined MAUL_REGIONINFO_LOCAL (
  if exist "%~dp0regioninfo.json" (
    set "MAUL_REGIONINFO_LOCAL=%~dp0regioninfo.json"
  ) else (
    set "MAUL_REGIONINFO_LOCAL=%~dp0data\regioninfo.json"
  )
)
...
echo.
echo Region source:
if exist "%MAUL_REGIONINFO_LOCAL%" (
  echo   %MAUL_REGIONINFO_LOCAL%
) else (
...
if exist "%MAUL_REGIONINFO_LOCAL%" (
  echo Using regioninfo.json next to this installer...
  copy /Y "%MAUL_REGIONINFO_LOCAL%" "%TEMP_FILE%" >nul
  if errorlevel 1 goto download_failed
) else (
  echo Downloading the latest MAUL regioninfo.json...
  powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -UseBasicParsing -Uri $env:MAUL_REGIONINFO_URL -OutFile $env:TEMP_FILE"
  if errorlevel 1 (
    echo PowerShell download failed. Trying curl...
    curl.exe -L --fail --output "%TEMP_FILE%" "%MAUL_REGIONINFO_URL%"
    if errorlevel 1 goto download_failed
  )
)

echo Checking the region file...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $json = Get-Content -LiteralPath $env:TEMP_FILE -Raw | ConvertFrom-Json; if (-not $json.Regions) { throw 'regionInfo.json is missing the Regions list.' }; $names = @($json.Regions | ForEach-Object { $_.Name }); if (-not ($names -match '^MAUL')) { Write-Warning 'No MAUL region names were found, but the file is valid JSON.' }"
...
echo Check your internet connection, or put regioninfo.json in a data folder next to this installer and run it again.
echo.
pause
exit /b 1
