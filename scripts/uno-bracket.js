(function () {
  ...
  function matchPlayerNames(match, map) {
    return {
      firstName: getParticipantLabel(map, match.playerOneId, match.playerOneName, match, 1),
      secondName: getParticipantLabel(map, match.playerTwoId, match.playerTwoName, match, 2)
    };
  }

  function sameDisplayName(first, second) {
    const firstPlain = stripToPlainName(first).toLowerCase();
    const secondPlain = stripToPlainName(second).toLowerCase();
    return Boolean(firstPlain && secondPlain && firstPlain === secondPlain);
  }

  function matchWinnerInfo(match, map) {
    const { firstName, secondName } = matchPlayerNames(match, map);
    const status = matchStatus(match);

    if (status === "cancelled" || !match.finalized) {
      return {
        firstName,
        secondName,
        slot: 0,
    ...
    if (match.winnerId && match.winnerId === match.playerOneId) {
      return {
        firstName,
        secondName,
        slot: 1,
    ...
    if (match.winnerId && match.winnerId === match.playerTwoId) {
      return {
        firstName,
        secondName,
        slot: 2,
    ...
    const rosterWinner = map.get(toId(match.winnerId));
    const fallbackWinner = cleanDisplayLabel(match.winnerName, match.winnerId);
    const winnerName = rosterWinner ? rosterWinner.displayName : fallbackWinner;
    ...
    if (sameDisplayName(winnerName, firstName)) {
      return {
        firstName,
        secondName,
        slot: 1,
    ...
    if (sameDisplayName(winnerName, secondName)) {
      return {
        firstName,
        secondName,
        slot: 2,
  ...
  function renderBracketMatch(match, map) {
    const winnerInfo = matchWinnerInfo(match, map);
    const status = matchStatus(match);
    ...
    return `
      <article class="match-card bracket-match bracket-match-compact" data-state="${status}">
        ...
        ${renderSlot(winnerInfo.firstName, winnerInfo.slot === 1)}
        ${renderSlot(winnerInfo.secondName, winnerInfo.slot === 2)}
      </article>
    `;
  }
  ...
  function projectedWinnerFromMatch(match, map) {
    const winnerInfo = matchWinnerInfo(match, map);
  ...
  function buildProjectedSlots(rounds, map) {
    const projectedSlots = rounds.map((round) =>
      round.slots.map((match) => (match ? projectedWinnerFromMatch(match, map) : null))
    );

    for (let roundIndex = 1; roundIndex < rounds.length; roundIndex += 1) {
      rounds[roundIndex].slots.forEach((match, slotIndex) => {
        if (match) {
          projectedSlots[roundIndex][slotIndex] = projectedWinnerFromMatch(match, map);
          return;
        }

        const previousRound = projectedSlots[roundIndex - 1] || [];
        const advancers = [previousRound[slotIndex * 2], previousRound[slotIndex * 2 + 1]]
          .filter((entry) => entry && entry.name)
          .map((entry) => entry.name);

        projectedSlots[roundIndex][slotIndex] = advancers.length
          ? {
              advancers
            }
          : null;
  ...
  function renderRoundPlaceholder(round, projection) {
    const advancers = projection && Array.isArray(projection.advancers)
      ? projection.advancers
      : [];

    if (advancers.length) {
      const slots = [advancers[0] || "TBD", advancers[1] || "TBD"];

      return `
        <article class="bracket-placeholder is-projected" aria-label="${escapeHtml(round.title)} channel waiting">
          <div class="placeholder-kicker">Awaiting channel</div>
          ${slots
            .map((name) => `
              <div class="projected-player${name === "TBD" ? " is-empty" : ""}">
                <span class="slot-name" title="${escapeHtml(name)}">${escapeHtml(name)}</span>
              </div>
            `)
            .join("")}
  ...
  function renderRoundColumn(round, map, projectedSlots) {
      ...
      >
        ...
        <div class="bracket-lineup">
          ${round.slots
            .map((match, slotIndex) =>
              match
                ? renderBracketMatch(match, map)
                : renderRoundPlaceholder(
                    round,
                    projectedSlots && projectedSlots[round.index]
                      ? projectedSlots[round.index][slotIndex]
                      : null
                  )
            )
            .join("")}
  ...
  function resolveChampion(map) {
    const explicitChampion = map.get(toId(state.championId));
    const explicitChampionName =
      cleanDisplayLabel(state.championName, state.championId) ||
      (explicitChampion ? explicitChampion.displayName : "");

    if (explicitChampionName) {
      return {
        name: explicitChampionName,
        gameId: "",
        source: "official"
      };
    }
    ...
    const latestMatch = state.matches
      .slice()
      .sort((first, second) => second.gameId - first.gameId)[0];
    const winnerInfo = latestMatch ? matchWinnerInfo(latestMatch, map) : null;

    if (
      !winnerInfo ||
      !winnerInfo.name ||
      matchStatus(latestMatch) === "cancelled" ||
      !latestMatch.finalized
    ) {
      return null;
    }
    ...
    return {
      ...
      source: "latest-finalized"
    };
  }

  function renderChampionAnnouncement(champion) {
    return `
      <section class="champion-stage" aria-label="UNO tournament champion">
        <div class="champion-copy">
          <div class="section-kicker">Tournament Complete</div>
          <h2>UNO Tournament 2.0 Champion</h2>
          <div class="champion-name" title="${escapeHtml(champion.name)}">${escapeHtml(champion.name)}</div>
          <p>Congratulations to ${escapeHtml(champion.name)} for winning the tournament.</p>
        </div>
        ${champion.gameId ? `
          <div class="champion-card">
            <span>Final Result</span>
            <strong>Game #${escapeHtml(champion.gameId)}</strong>
          </div>
        ` : ""}
      </section>
    `;
  ...
  function renderBracket() {
    ...
    const champion = resolveChampion(map);

    if (champion) {
      elements.bracket.innerHTML = renderChampionAnnouncement(champion);
      return;
    }
    ...
    const projectedSlots = buildProjectedSlots(rounds, map);
      ...
      <section class="bracket-stage">
        <div class="bracket-stage-head">
          <div>
            ...
            <p>Created channels are shown as game cards. Known winners flow into neutral next-round slots until those Discord channels exist.</p>
          </div>
          <div class="board-count">
        ...
        <div class="bracket-track bracket-clean">
          ${rounds.map((round) => renderRoundColumn(round, map, projectedSlots)).join("")}
        </div>
      </section>
    `;
  }
  ...
  function renderMatches() {
    ...
    elements.matchList.innerHTML = state.matches
      ...
      .map((match) => {
        ...
        const status = matchStatus(match);
        const winnerInfo = matchWinnerInfo(match, participantMap);
        const winnerName = winnerInfo.name || (status === "cancelled" ? "No winner" : "Pending");
  ...
  async function loadData() {
    ...
    state = {
      ...
      tournamentComplete: Boolean(
        rawData.tournament_complete ||
        rawData.event_complete ||
        rawData.completed ||
        (rawData.event && rawData.event.completed)
      ),
      championId: toId(
        rawData.champion_id ||
        rawData.winner_id ||
        (rawData.event && rawData.event.champion_id)
      ),
      championName: firstText(
        rawData.champion_name,
        rawData.winner_name,
        rawData.final_winner_name,
        rawData.finalWinnerName,
        rawData.event && rawData.event.champion_name
      )
    };
