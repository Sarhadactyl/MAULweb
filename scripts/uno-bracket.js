(function () {
  ...
  function renderActualMatchCard(match, map) {
    ...
    return `
      <article class="match-card" data-state="${status}">
  ...
  function renderBracket() {
    ...
    if (!state.matches.length) {
      elements.bracket.innerHTML = `
        <section class="board-section board-hero">
          <div>
            <div class="section-kicker">Waiting for match channels</div>
            <h2>No match channels yet</h2>
            <p>
              The roster is loaded, and public games will appear here after staff opens
              real Discord match channels.
            </p>
          </div>
    ...
    elements.bracket.innerHTML = `
      <section class="board-section">
        <div class="section-heading">
          <h2>Open Games</h2>
          <span>${openMatches.length} active</span>
        </div>
        <div class="match-board-grid">
          ${
            openMatches.length
              ? openMatches.map((match) => renderActualMatchCard(match, map)).join("")
              : `<div class="empty-state"><strong>No open games.</strong><br />Created match channels will appear here.</div>`
          }
        </div>
      </section>
      <section class="board-section">
        <div class="section-heading">
          <h2>Results</h2>
          <span>${completedMatches.length} finished</span>
        </div>
        <div class="match-board-grid">
          ${
            completedMatches.length
              ? completedMatches.map((match) => renderActualMatchCard(match, map)).join("")
              : `<div class="empty-state"><strong>No results yet.</strong><br />Finalized games will appear here.</div>`
          }
        </div>
      </section>
